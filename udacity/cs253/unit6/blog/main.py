...
class BlogHandler(webapp2.RequestHandler):
    ...
    def memcache_getfront(self, update=False):
        memcache_key='front'
        pt=memcache.get(memcache_key)
        if pt is not None and not update:
            return [pt[0], pt[1], int(time.mktime(time.gmtime())-pt[2])]
        else:
            ...
            if not memcache.set(memcache_key, [posts,site_title,ctime]):
                logging.error('Memcache set failed')
            return [posts,site_title,int(time.mktime(time.gmtime())-ctime)]

    def memcache_getpost(self,pid,update=False):
        memcache_key='post'+str(pid)
        pt=memcache.get(memcache_key)
        if pt is not None and not update:
            logging.error('sdafaf'+repr(pt[0]))
            return [pt[0], pt[1], int(time.mktime(time.gmtime())-pt[2])]
        else:
            try:
                ...
                posts=None
                logging.error('asdfasf')
                return [None, None, None]
            if not key or not posts:
                return [None, None, None]
            posts=[posts]
            logging.error('asdfadsf')
            site_title=db.GqlQuery('select * from Meta').fetch(1)[0].site_title
            ctime=time.mktime(time.gmtime())
            if not memcache.add(memcache_key, [posts,site_title,ctime]):
                logging.error('Memcache set failed')
            return [posts,site_title,int(time.mktime(time.gmtime())-ctime)]
...
class BlogNew(BlogHandler):
    ...
    def post(self):
        ...
        if subject and content:
            ...
            post.put()
            pid=post.key().id()
            self.memcache_getfront(update=True)
...
class Blog(BlogHandler):    
    def get(self):
        [posts, site_title, ages]=self.memcache_getfront()
        if ages==1:
            ages=str(ages)+' second'
        else:
            ages=str(ages)+' seconds'
        self.render('blog.html',posts=posts,site_title=site_title,ages=ages)

class BlogPage(BlogHandler):    
    def get(self, pid):
        [posts, site_title, ages]=self.memcache_getpost(pid)
        if not posts:
            self.error(404)
