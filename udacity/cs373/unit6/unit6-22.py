...
def slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    tN = N+num_landmarks
    omega = matrix()
    omega.zero(2*tN,2*tN)
    ksi = matrix()
    ksi.zero(2*tN,1)
    
    confidence_motion = 1./((motion_noise)**2)
    confidence_measurement = 1./((measurement_noise)**2)
    
    inc_o = matrix()
    inc_o.zero(2*tN,2*tN)
    inc_x = matrix()
    inc_x.zero(2*tN,1)
    
    #init position
    inc_o.value[0][0] = inc_o.value[1][1] = 1.
    inc_x.value[0][0] = inc_x.value[1][0] = world_size/2.0
    omega += inc_o
    ksi += inc_x
    ...
    for i in range(len(data)):
        ...
        for j in range(len(zi)):
            ...
            inc_o.zero(2*tN,2*tN)
            inc_x.zero(2*tN,1)
            inc_o.value[xi][xi]+=confidence_measurement
            inc_o.value[xi][lxi]+= -confidence_measurement
            inc_x.value[xi][0]+= -confidence_measurement*zi[j][1]
            inc_o.value[yi][yi]+=confidence_measurement
            inc_o.value[yi][lyi]+= -confidence_measurement
            inc_x.value[yi][0]+= -confidence_measurement*zi[j][2]
            inc_o.value[lxi][lxi]+=confidence_measurement
            inc_o.value[lxi][xi] += -confidence_measurement
            inc_x.value[lxi][0]+= confidence_measurement*zi[j][1]
            inc_o.value[lyi][lyi]+=confidence_measurement
            inc_o.value[lyi][yi] += -confidence_measurement
            inc_x.value[lyi][0]+= confidence_measurement*zi[j][2]
            omega += inc_o
            ksi += inc_x
            
        inc_o.zero(2*tN, 2*tN)
        inc_x.zero(2*tN,1)
        inc_o.value[xi][xi]+=confidence_motion
        inc_o.value[xi][xi+2]+=-confidence_motion
        inc_x.value[xi][0]+= -confidence_motion*mi[0]
        inc_o.value[yi][yi]+=confidence_motion
        inc_o.value[yi][yi+2]+=-confidence_motion
        inc_x.value[yi][0]+= -confidence_motion*mi[1]
        inc_o.value[xi+2][xi+2]+=confidence_motion
        inc_o.value[xi+2][xi]+=-confidence_motion
        inc_x.value[xi+2][0]+= confidence_motion*mi[0]
        inc_o.value[yi+2][yi+2]+=confidence_motion
        inc_o.value[yi+2][yi]+=-confidence_motion
        inc_x.value[yi+2][0]+= confidence_motion*mi[1]
        omega += inc_o
        ksi+= inc_x
