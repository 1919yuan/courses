...
## The following part uses Nelder-Mead algorithm to find an optimum
## The program may be terminated by the server if max_iter is too large
## One possible answer with max_iter=10 and gamma = 0.05 on my own machine
## obtains the result (0 collision, 143 time step)
#weight_data =  0.080637775927986349
#weight_smooth = 0.16449553164132716
#p_gain = 1.4439264800802236
#d_gain = 7.8658866447389784

## Another possible answer with max_iter=25 and gamma = 1 gives (28 collision and 76 time step)
weight_data =  -0.00094448083
weight_smooth = 0.0258650
p_gain = 2.1948250
d_gain = 19.92521930


###The following is the Nelder-Mead algorithm 
##simplex = []
##gamma = 0.1
##max_iter = 5
##param = [weight_data,weight_smooth,p_gain,d_gain]
##for i in range(len(param)+1):
##    new_param = []
##    for j in range(len(param)):
##        new_param.append(param[j]*random.uniform(0.5,1.5))
##    cost = main(grid, init, goal, steering_noise, distance_noise, measurement_noise, new_param[0],new_param[1],new_param[2],new_param[3])
##    f = cost[1] + gamma * cost[2]
##    simplex.append([f,new_param])
##    
##simplex_n = len(simplex)
##simplex_dim = len(simplex[0][1])
##for it in range(max_iter):
##    simplex.sort()
##    centroid = [0.] * simplex_dim
##    step = 0
##    f_new = 0
##    for j in range(simplex_n-1):
##        for k in range(simplex_dim):
##            centroid[k]+=simplex[j][1][k]/(simplex_n-1)
##    middle = [0.] * simplex_dim
##    for k in range(simplex_dim):
##        middle[k] = 2*centroid[k]-simplex[-1][1][k]
##    cost = main(grid, init, goal, steering_noise, distance_noise, measurement_noise, middle[0],middle[1],middle[2],middle[3])
##    f_middle = cost[1] + gamma * cost[2]
##    if (simplex[0][0] <= f_middle and f_middle <= simplex[-2][0]):
##        x_new = middle
##        f_new = f_middle
##        step = -1
##    elif (simplex[0][0] > f_middle):
##        middle2 = [0.]*simplex_dim
##        for k in range(simplex_dim):
##            middle2[k] = 3*centroid[k]-2*simplex[-1][1][k]
##        cost = main(grid, init, goal, steering_noise, distance_noise, measurement_noise, middle2[0],middle2[1],middle2[2],middle2[3])
##        f_middle2 = cost[1] + gamma * cost[2]
##        if f_middle2 < f_middle:
##            x_new = middle2
##            f_new = f_middle2
##            step = -2
##        else:
...
##    else:
##        step = 0
##        if (simplex[-2][0] <= f_middle and f_middle < simplex[-1][0]):
##            middle3 = [0.]*simplex_dim
##            for k in range(simplex_dim):
##                middle3[k] = 1.5*centroid[k]-0.5*simplex[-1][1][k]
##            cost = main(grid, init, goal, steering_noise, distance_noise, measurement_noise, middle3[0],middle3[1],middle3[2],middle3[3])
##            f_middle3 = cost[1] + gamma * cost[2]
##            if f_middle3 <= f_middle:
##                step = -0.5
##                x_new = middle3
##                f_new = f_middle3
##        else:
##            middle4 = [0.]*simplex_dim
##            for k in range(simplex_dim):
##                middle4[k] = 0.5*centroid[k]+0.5*simplex[-1][1][k]
##            cost = main(grid, init, goal, steering_noise, distance_noise, measurement_noise, middle4[0],middle4[1],middle4[2],middle4[3])
##            f_middle4 = cost[1] + gamma * cost[2]
##            if f_middle4 < simplex[-1][0]:
##                step = 0.5
##                x_new = middle4
##                f_new = f_middle4    
##    if step != 0:
##        simplex.pop()
##        simplex.append([f_new,x_new])
##    else:
##        for j in range(1,simplex_n):
##            for k in range(simplex_dim):
##                simplex[j][1][k] = 0.5*simplex[j][1][k]+0.5*simplex[0][1][k]
##            cost = main(grid, init, goal, steering_noise, distance_noise, measurement_noise, simplex[j][1][0],simplex[j][1][1],simplex[j][1][2],simplex[j][1][3])
##            f = cost[1] + gamma * cost[2]
##            simplex[j][0] = f
##            
##simplex.sort()
##weight_data = simplex[0][1][0]
##weight_smooth = simplex[0][1][1]
##p_gain = simplex[0][1][2]
##d_gain = simplex[0][1][3]
