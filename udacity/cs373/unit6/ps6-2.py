...
def online_slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    ksi = matrix()
    ksi.zero(dim,1)
    omega.value[0][0] = omega.value[1][1] = 1.
    ksi.value[0][0] = world_size / 2.0
    ksi.value[1][0] = world_size / 2.0
    ...
    for i in range(len(data)):
        ...
        for j in range(len(zi)):
            ...
            inc_o.zero(dim,dim)
            inc_x.zero(dim,1)
            inc_o.value[xi][xi]+=1./measurement_noise
            inc_o.value[xi][lxi]-=1./measurement_noise
            inc_x.value[xi][0]-=1./measurement_noise*zi[j][1]
            inc_o.value[lxi][lxi]+=1./measurement_noise
            inc_o.value[lxi][xi]-=1./measurement_noise
            inc_x.value[lxi][0]+=1./measurement_noise*zi[j][1]
            inc_o.value[yi][yi]+=1./measurement_noise
            inc_o.value[yi][lyi]-=1./measurement_noise
            inc_x.value[yi][0]-=1./measurement_noise*zi[j][2]
            inc_o.value[lyi][lyi]+=1./measurement_noise
            inc_o.value[lyi][yi]-=1./measurement_noise
            inc_x.value[lyi][0]+=1./measurement_noise*zi[j][2]
            omega += inc_o
            ksi += inc_x
            
        inc_o.zero(dim,dim)
        inc_x.zero(dim,1)
        inc_o.value[xi][xi]+=1./motion_noise
        inc_o.value[xi][xi+2]-=1./motion_noise
        inc_x.value[xi][0]-=1./motion_noise*mi[0]
        inc_o.value[xi+2][xi+2]+=1./motion_noise
        inc_o.value[xi+2][xi]-=1./motion_noise
        inc_x.value[xi+2][0]+=1./motion_noise*mi[0]
        inc_o.value[yi][yi]+=1./motion_noise
        inc_o.value[yi][yi+2]-=1./motion_noise
        inc_x.value[yi][0]-=1./motion_noise*mi[1]
        inc_o.value[yi+2][yi+2]+=1./motion_noise
        inc_o.value[yi+2][yi]-=1./motion_noise
        inc_x.value[yi+2][0]+=1./motion_noise*mi[1]
        omega += inc_o
        ksi += inc_x
        ...
        ksi_prime = ksi.take(range(2,dim),[0])
        A = omega.take([0,1],range(2,dim))
        B = omega.take([0,1],[0,1])
        C = ksi.take([0,1],[0])
        D = A.transpose() * B.inverse()
        omega_prime = omega_prime - D*A
        ksi_prime = ksi_prime - D*C
        for ki in ([0,1]):
            for kj in ([0,1]):
                omega.value[ki][kj] = omega_prime.value[ki][kj]
        for ki in range(4,dim):
            for kj in range(4,dim):
                omega.value[ki][kj] = omega_prime.value[ki-2][kj-2]
        for ki in ([0,1]):
            for kj in range(4,dim):
                omega.value[ki][kj] = omega_prime.value[ki][kj-2]
        for kj in ([0,1]):
            for ki in range(4,dim):
                omega.value[ki][kj] = omega_prime.value[ki-2][kj]
        for ki in ([2,3]):
            for kj in range(dim):
                omega.value[ki][kj] = 0.
        for kj in ([2,3]):
            for ki in range(dim):
                omega.value[ki][kj]=0.
        ksi.value[0][0] = ksi_prime.value[0][0]
        ksi.value[1][0] = ksi_prime.value[1][0]
        for ki in range(4,dim):
            ksi.value[ki][0] = ksi_prime.value[ki-2][0]
        ksi.value[2][0] = ksi.value[3][0] = 0.
