...
class robot:
    ...
    def move(self, motion): # Do not change the name of this function
        ...
        # ADD CODE HERE        
        d=float(motion[1])+random.gauss(0.,self.distance_noise)
        alpha=float(motion[0])+random.gauss(0.,self.steering_noise)
        ...
        if (fabs(beta)>=0.001):
            ...
            result.orientation = self.orientation + alpha
            result.orientation %= 2 * pi            
            # move, and add randomness to the motion command
            result.x = self.x + (cos(result.orientation) * d)
            result.y = self.y + (sin(result.orientation) * d)
    ...
    def sense(self,add_noise=False): #do not change the name of this function
        ...
        for i in range(len(landmarks)):
            z=atan2(landmarks[i][0]-self.y,landmarks[i][1]-self.x)-self.orientation
            if (add_noise):
