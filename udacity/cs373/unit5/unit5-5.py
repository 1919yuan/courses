    ...
    while (total_change > tol):
        ...
        for i in range(1,len(path)-1):
            ...
            for j in range(len(path[0])):
                ...
                newpath[i][j] += inc
                inc = weight_smooth * (newpath[i+1][j]+newpath[i-1][j]-2*newpath[i][j])
                newpath[i][j] += inc
                change += (prev-newpath[i][j])**2
            total_change += sqrt(change)
        total_change /= len(path)-2
